...
# Définition automatique des couleurs selon le thème
...
# Sidebar widgets : fond blanc et texte bleu fixe
...
# Fonction pour créer tous les graphiques avec fond automatique
def plot(fig):
    fig.update_layout(plot_bgcolor=page_bg, paper_bgcolor=page_bg, title_font=dict(color=header_color, size=18))
    st.plotly_chart(fig, use_container_width=True)

# --- Répartition par type de dépôt ---
type_counts = df_filtered["Type_depot"].value_counts().sort_values()
fig_type = px.bar(x=type_counts.index, y=type_counts.values, text=type_counts.values, template=plotly_template, height=400)
fig_type.update_layout(xaxis_title="Type de dépôt", yaxis_title="Nombre de griefs")
plot(fig_type)

# --- Avancement général ---
fig_stat = px.pie(df_filtered, names="Statut_traitement", color="Statut_traitement",
                  color_discrete_map=colors_map_statut, template=plotly_template, height=400)
...
fig_nature = px.histogram(df_filtered, y="Nature_plainte", color="Statut_traitement", text_auto=True,
                          ...
                          color_discrete_map=colors_map_statut, template=plotly_template, height=400)
fig_nature.update_layout(xaxis_title="Nature de griefs", yaxis_title="Nombre", legend_title_text="Statut du traitement")
plot(fig_nature)
...
if "Nb_jour" in df_trim.columns:
    ...
    fig_duree = px.bar(df_duree, x="Nature_plainte", y="Nb_jour", text_auto=".1f",
                       template=plotly_template, height=400)
    fig_duree.update_layout(xaxis_title="Nature de griefs", yaxis_title="Durée (jours)")
    plot(fig_duree)
