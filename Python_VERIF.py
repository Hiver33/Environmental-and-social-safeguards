...
for col,(val,label),color in zip(cols,metrics,colors):
    col.markdown(f"<div style='background:{color}; padding:15px; border-radius:15px;'><p style='font-size:28px;color:black'>{val}</p><p style='color:black'>{label}</p></div>", unsafe_allow_html=True)
...
fig_type = px.bar(x=type_counts.index,y=type_counts.values,text=type_counts.values,title="Répartition par type de dépôt",template="plotly_dark",height=400)
# Statut avec vert pour Achevé
...
fig_stat = px.pie(df_filtered,names="Statut_traitement",title="Avancement général",color="Statut_traitement",
                  color_discrete_map=colors_map,template="plotly_dark",height=400)
...
fig_nature = px.histogram(df_filtered,y="Nature_plainte",color="Statut_traitement",text_auto=True,
                          category_orders={"Nature_plainte":ordre_nature},orientation="h",template="plotly_dark",height=400)
...
fig_comm = px.bar(x=comm_counts.index,y=comm_counts.values,text=comm_counts.values,title="Nombre de griefs par communauté",template="plotly_dark",height=400)
...
fig_cat_sexe = px.bar(df_cat_sexe,y="Nature_plainte",x="Nombre",color="Sexe",category_orders={"Nature_plainte":ordre_nature_tri},orientation="h",template="plotly_dark",height=400)
...
fig_line = px.line(df_line,x="Mois",y="Nombre",color="Nature_plainte",markers=True,title=f"Top {top_n} évolution",template="plotly_dark",height=400)
...
if "Nb_jour" in df_trim.columns:
    ...
    fig_duree = px.bar(df_duree,x="Nature_plainte",y="Nb_jour",text_auto=".1f",title="Durée moyenne par nature",template="plotly_dark",height=400)
