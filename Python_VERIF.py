...
Types = st.sidebar.multiselect(
    "📂 Type de dépôt :", df["Type_depot"].unique(), default=df["Type_depot"].unique()
)
Statuts = st.sidebar.multiselect(
    "✅ Statut de traitement :", df["Statut_traitement"].unique(), default=df["Statut_traitement"].unique()
)
...
cols = st.columns(4)
metrics = [(total,"Total des griefs"),(acheves,"Achevés"),(en_cours,"En cours"),(non_traites,"Non traités")]
...
for col,(val,label),color in zip(cols,metrics,colors):
    col.markdown(
        ...
        f"<p style='font-weight:bold;color:black'>{label}</p></div>",
        unsafe_allow_html=True
    )
