...
df["Statut_traitement_clean"] = df["Statut_traitement"].str.strip()
...
plein_ecran = st.sidebar.toggle("🖥️ Plein écran automatique")
...
fig_type = px.bar(x=type_counts.index, y=type_counts.values, text=type_counts.values,
                  title="Répartition par type de dépôt", template="plotly_dark")
fig_type.update_traces(textposition="outside")
fig_type.update_layout(autosize=True, height=600)
...
fig_stat = px.pie(df_filtered, names="Statut_traitement_clean", title="Avancement général des griefs",
                  color="Statut_traitement_clean", color_discrete_map=colors_map, template="plotly_dark")
fig_stat.update_traces(textinfo="percent+label", textposition="inside")
fig_stat.update_layout(autosize=True, height=600)
...
fig_nature.update_layout(autosize=True, height=600)
...
fig_comm = px.bar(x=comm_counts.index, y=comm_counts.values, text=comm_counts.values,
                  title="Nombre de griefs par communauté", template="plotly_dark")
fig_comm.update_traces(marker_color="#00ccff", textposition="outside")
fig_comm.update_layout(autosize=True, height=600)
fig_sexe = px.pie(df_filtered, names="Sexe", title="Répartition par sexe", template="plotly_dark")
fig_sexe.update_traces(textinfo="percent+label", textposition="inside")
fig_sexe.update_layout(autosize=True, height=600)
...
fig_cat_sexe = px.bar(df_cat_sexe, y="Nature_plainte", x="Nombre", color="Sexe",
                      ...
                      orientation="h", template="plotly_dark", text="Nombre")
fig_cat_sexe.update_traces(textposition="inside")
fig_cat_sexe.update_layout(autosize=True, height=600)
...
fig_line = px.line(df_line, x="Mois", y="Nombre", color="Nature_plainte", markers=True,
                   title=f"Top {top_n} évolution", template="plotly_dark")
fig_line.update_xaxes(dtick="M1", tickformat="%b", tickangle=-45)
fig_line.update_layout(autosize=True, height=600)
...
if "Nb_jour" in df_trim.columns:
    ...
    fig_duree = px.bar(df_duree, x="Nature_plainte", y="Nb_jour", text_auto=".1f",
                       title="Durée moyenne par nature", template="plotly_dark")
    fig_duree.update_traces(textposition="outside")
    fig_duree.update_layout(autosize=True, height=600)
